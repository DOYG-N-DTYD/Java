...
4) identity replaced with id bigint generated by default as identity primary key		

5) NOT WORKING (only with constructor ingredient and changed method addIngredientsToModel) -> design template




6) SecurityConfig depreciation -> change from documentation
7 Development config -> issue
8) security config -> issue