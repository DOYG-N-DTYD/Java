...
public class AuthorisationPanel {
    ...
    private void addMessageToChat() {
        ...
        for (int i = 0; i < chatStack.size(); i++) {
            if (chatStack.size() < 10) { // 10 rows in chat gui
                StringBuilder chatMsg = new StringBuilder();
                cleanAllNewLines();
                for (int j = 0; j < 10 - chatStack.size(); j++) { // add to last msg 10 - x "\n"
                    chatMsg.append("\n");
                }
                chatMsg.append(chatStack.get(i));
                msgString.append(chatMsg);
            } else {
    ...
    private void cleanAllNewLines() {
        for (int i = 0; i < chatStack.size(); i++) {
            chatStack.get(i).replaceAll("\n", "");
        }
    }
}
